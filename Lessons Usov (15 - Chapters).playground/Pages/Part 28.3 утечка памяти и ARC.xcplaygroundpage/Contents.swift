...
// Утечка памяти и ARC
// При утечке памяти Swift неможет принять своевременное решение об удалении объектов в куче. В результате этого в памяти появляются "бесхозные" записи, которые никогда больше не будут использоваться и будут находится там до завершения работы приложения
