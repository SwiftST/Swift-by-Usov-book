...
// Изменяемость - экземпляр класса может изменять значения своих свойств объявленных в классе как переменная, даже если сам экземпляр хранится в константе. Использование ключевого слово mutating для методов не требуется
